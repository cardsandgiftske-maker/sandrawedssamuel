  ...
  return (
    <>
      ...
      <iframe
        ...
        width="1"
        height="1"
        src={`https://www.youtube.com/embed/9YcrMk_cZyI?enablejsapi=1&autoplay=0&controls=0&loop=1&playlist=9YcrMk_cZyI&origin=${typeof window !== 'undefined' ? encodeURIComponent(window.location.origin) : ''}`}
        ...
        {shouldPlay && (
          ...
          >
            <div className="flex items-center gap-2 bg-white/95 backdrop-blur-md border border-stone-200/80 rounded-full p-1.5 shadow-[0_8px_30px_rgb(0,0,0,0.06)] hover:shadow-[0_10px_35px_rgb(0,0,0,0.1)] hover:border-stone-300 transition-all group">
              ...
              <button
                onClick={togglePlay}
                className="w-10 h-10 rounded-full bg-[#FFF0F3] border border-[#E892A2]/40 flex items-center justify-center text-[#722F37] hover:bg-[#722F37] hover:text-white transition-all cursor-pointer relative overflow-hidden group/btn"
              ...
              <div className="flex items-center gap-1.5 pr-3 pl-1 font-serif text-xs font-medium tracking-tight text-stone-700 select-none">
                <span className="hidden sm:inline-block max-w-[120px] truncate text-[#722F37]">Sandra &amp; Samuel Song</span>
                
                <div className="w-[1px] h-4 bg-stone-200 mx-1 hidden sm:block" />

                <button
                  onClick={toggleMute}
                  className="p-1 text-stone-500 hover:text-[#722F37] active:scale-90 transition-all cursor-pointer"
                  title={isMuted ? "Unmute Music" : "Mute Music"}
                >
                  {isMuted ? (
                    <VolumeX className="w-4 h-4 text-[#722F37]" />
                  ) : (
